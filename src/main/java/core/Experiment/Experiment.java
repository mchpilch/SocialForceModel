...
public class Experiment extends Game {
    ...
    Array<Pillar> pillarStorage = new Array<Pillar>();
    ...
    public void create() {
        ...
        Pillar pillar1 = new Pillar(0,0,0.4f,world);
        pillarStorage.add(pillar1);
...
//        }
    ...
    public Vector2 calculateNetWallForce(Human pedestrian, float wallRepNomCoeff, float wallRepNomCoeff2, int powerR){
        ...
        for(int i = 0; i < pillarStorage.size; i++){
            float pilX = pillarStorage.get(i).getX();
            float pilY = pillarStorage.get(i).getY();

            float pedX = pedestrian.body.getPosition().x;
            float pedY = pedestrian.body.getPosition().y;

            Vector2 vecPil = new Vector2(pilX - pedX,pilY - pedY);
            float distance = vecPil.len();
            vecPil = vecPil.nor();
            vecPil = vecPil.scl(pillarCoeff/distance*distance);
            netWallForce.add(vecPil);
        }
        return  netWallForce;
    }
